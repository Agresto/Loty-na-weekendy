 ...
 * generate-flights.js
 * ─────────────────────────────────────────────────────────────────────────────
 * Generuje flights.json z kompaktowych reguł routes × dates.
 *
 * Użycie:
 ...
 *   • Zapisuje do flights.json z timestamp ostatniej aktualizacji
 *
 * UWAGA - PRODUKCJA:
 *   W produkcji ten skrypt jest fallbackiem dla refresh-flights.js, który
 *   pobiera prawdziwe ceny z:
 *     • Ryanair farfnd API:  https://services-api.ryanair.com/farfnd/v4/...
 *     • Wizzair public API:  https://be.wizzair.com/.../timetable
 *   Patrz: refresh-flights.js i .github/workflows/refresh-flights.yml
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
const DESTS = {
  ...
  LCA: {city:'Larnaka (Cypr)',   country:'Cypr',             flag:'🇨🇾',distKm:9, passport:false,visa:'brak',  currency:'EUR €',  englishOk:true, sea:true, lgbt:false,lgbtN:'Cypr – homoseksualność zdekryminalizowana, ale ograniczone prawa partnerskie'},
  LTN: {city:'Londyn Luton',     country:'Wielka Brytania',  flag:'🇬🇧',distKm:60,passport:true, visa:'brak',  currency:'GBP £',  englishOk:true, sea:false,lgbt:true, lgbtN:'Londyn – jedna z najbardziej LGBTQ+ friendly metropolii'},
};
...
const ROUTES = [
  ...
  ['KTW','MAD','wizzair', 219, 230, 13],
  // ── KRK ──
  ['KRK','MAD','ryanair', 229, 230, 12],
  ['KRK','LCA','wizzair', 249, 260, 6],
  ['KRK','AGP','wizzair', 219, 215, 8],
...
function adjustPrice(base, date, pattern) {
  ...
  let factor = 1.0;
  if (month === 7 || month === 8) factor = 1.3;
  else if (month === 12) factor = 1.4;
  else if (month === 1 || month === 2 || month === 11) factor = 0.85;
  else if (month === 6 || month === 9) factor = 1.15;
...
async function generateFlights(today = new Date()) {
  ...
  for (const route of ROUTES) {
    ...
    for (const dInfo of pickedDates) {
      ...
      const idPrefix = airline === 'wizzair' ? 'w' : 'r';
      flights.push({
        id:        `${idPrefix}${id++}`,
        airline,
        from:      origin,
