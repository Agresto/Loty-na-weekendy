 ...
 *
 * ── RYANAIR ──────────────────────────────────────────────────────────────────
 * Poprawny format (wersja booking flow, 2024+):
 ...
 *     &dateIn=               ← puste = lot w jedną stronę
 *     &isReturn=false
 *     &discount=0
 *     &promoCode=
 ...
 *     &destinationIata=BCN
 *
 * Parametry &originIata i &destinationIata są redundantne ale wymuszają
 * poprawne pre-załadowanie lotnisk w React SPA Ryanair — bez nich
 * strona czasem ignoruje departureAirport przy hydration.
 *
 * ── WIZZAIR ──────────────────────────────────────────────────────────────────
 * Format REST URL (stabilny od 2022):
 *   https://wizzair.com/pl-pl/booking/select-flight
 *     /{FROM}/{TO}/{YYYY-MM-DD}/null/{ADULTS}/{CHILDREN}/{INFANTS}/{PROMO}
 *
 * ── UWAGA ────────────────────────────────────────────────────────────────────
 * Obie linie nie udostępniają publicznego API do linku do konkretnej godziny.
 * Link otwiera wyniki dla lotnisk + daty — użytkownik widzi dostępne loty i
 * wybiera ten z pasującą godziną (wyświetloną na przycisku w naszej aplikacji).
 *
...
function buildFlightUrl(airline, from, to, date) {
  if (airline === 'ryanair') {
    // Używamy URLSearchParams żeby uniknąć błędów kodowania
    const params = new URLSearchParams([
      ...
      ['children',           '0'],
      ['infants',            '0'],
      ['dateOut',            date],   // ← konkretna data wylotu YYYY-MM-DD
      ['dateIn',             ''],
      ['isReturn',           'false'],
      ['discount',           '0'],
      ['promoCode',          ''],
      ['isConnectedFlight',  'false'],
      ['originIata',         from],   // redundantne ale stabilizuje SPA
      ['destinationIata',    to],
    ]);
    return `https://www.ryanair.com/pl/pl/booking/home?${params.toString()}`;
  }

  // Wizzair — REST URL (stabilny format)
  // /select-flight/{FROM}/{TO}/{DATE}/{DATE_IN}/{ADU}/{CHI}/{INF}/{PROMO}
  // Lot w jedną stronę: dateIn = 'null'
